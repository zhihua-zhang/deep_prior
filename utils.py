...
def save_results(model, model_ema, metrics, epoch):
    ...
    torch.save(model.state_dict(), "./checkpoints/epoch={}-acc={:.3f}.pth".format(epoch, metrics["val"]["acc"][-1]))
    torch.save(model_ema.state_dict(), "./checkpoints/ema-epoch={}-acc={:.3f}.pth".format(epoch, metrics["val"]["ema_acc"][-1]))

    if os.path.exists("./logs"):
        os.system("rm -rf ./logs")
    os.makedirs("./logs", exist_ok=True)
    with open("./logs/epoch={}.json".format(epoch), "w") as f:
        json.dump(metrics, f)